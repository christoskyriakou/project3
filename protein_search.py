...
# --- ΒΕΛΤΙΩΜΕΝΗ ΦΟΡΤΩΣΗ ΓΙΑ BIO COMMENTS ---
def load_seq_db(fasta_path):
    ...
    for record in SeqIO.parse(fasta_path, "fasta"):
        ...
        # Προσπάθεια εξαγωγής καθαρού ονόματος πρωτεΐνης
...
def main():
    ...
    for i, qid in enumerate(query_ids):
        ...
        f_out.write("==============================================================================================================\n")
        f_out.write(f"\nQuery Protein: {qid}\n")
        f_out.write(f"N = {args.N} (μέγεθος λίστας Top-N για την αξιολόγηση Recall@N)\n\n")
        
        blast_hits, blast_t = run_blast(current_query_seq, qid, blast_db, args.N)
        blast_ids = set([h['id'] for h in blast_hits])
        
        f_out.write("[1] Συνοπτική σύγκριση μεθόδων\n")
        f_out.write("-" * 85 + "\n")
        ...
        f_out.write(f"[2] Top-N γείτονες ανά μέθοδο (N=10 για εκτύπωση)\n")
        for m in methods:
            res = method_results[m].get(i, [])
            f_out.write(f"\nMethod: {m}\n")
            f_out.write(f"{'Rank':<6}| {'Neighbor ID':<20} | {'L2Dist':<11}| {'Ident.':<9}| {'In BLAST?':<11}| {'Bio Comment'}\n")
            f_out.write("-" * 125 + "\n") # Μεγάλωσα λίγο το πλάτος
